...
def get_args():
    ...
    parser.add_argument('--online-bpe', action='store_true', help='use online byte pair encoding')
    parser.add_argument('--bpe-codes', default='bpe/bpe_fit/', help='path to bpe codes')
...
def main(args):
    ...
    # apply "normal" bpe if specified
    if args.bpe:
        apply_bpe(args, train_dataset)
        apply_bpe(args, valid_dataset)
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        # apply online bpe if specified
        if args.online_bpe and epoch > 0:
            apply_bpe(args, train_dataset)
            apply_bpe(args, valid_dataset)        
...
def apply_bpe(args, dataset):
    """ Applies BPE to a dataset. """
    
    bpe_code_path = os.path.join(args.bpe_codes, args.source_lang, 'bpe.codes')

    # Load BPE codes
    bpe_codes = codecs.open(bpe_code_path, encoding='utf-8')
    bpe = BPE(bpe_codes, separator='@@')
    
    # Apply BPE to dataset
    print(dataset['src_tokens'])
    in_file = dataset.src_file
    out_file = codecs.open(os.path.join(args.bpe_codes, args.source_lang, 'bpe.out'), 'w', encoding='utf-8')
    bpe.process_lines(in_file, out_file, dropout=args.bpe_dropout)
    in_file.close()
    out_file.close()
    
    # return the out file
    return codecs.open(os.path.join(args.bpe_codes, args.source_lang, 'bpe.out'), encoding='utf-8')
