...
def get_args():
    ...
    parser.add_argument('--bpe-dropout', type=float, default=0.1, help='BPE dropout rate during training')
...
def apply_bpe_dropout(tokens, dropout_rate, pad_idx, unk_idx):
    mask = (torch.rand(tokens.size()) < dropout_rate).to(tokens.device)
    mask &= (tokens != pad_idx)
    return torch.where(mask, torch.full_like(tokens, unk_idx), tokens)
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            if args.bpe_dropout > 0:
                sample['src_tokens'] = apply_bpe_dropout(sample['src_tokens'], args.bpe_dropout, src_dict.pad_idx, src_dict.unk_idx)
                sample['tgt_inputs'] = apply_bpe_dropout(sample['tgt_inputs'], args.bpe_dropout, tgt_dict.pad_idx, tgt_dict.unk_idx)
